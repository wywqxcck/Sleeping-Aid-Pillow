...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueMassage != cur->valueMassage)
    {
        GIZWITS_LOG("valueMassage Changed\n");
        ret = 1;
    }
    if(last->valueSleep != cur->valueSleep)
    {
        GIZWITS_LOG("valueSleep Changed\n");
        ret = 1;
    }
    ...
    if(last->valueHeart_rate != cur->valueHeart_rate)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueHeart_rate Changed\n");
            ret = 1;
        }
    }
    if(last->valueBlood_oxygen != cur->valueBlood_oxygen)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueBlood_oxygen Changed\n");
            ret = 1;
        }
    }
    if(last->valueSnoring != cur->valueSnoring)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueSnoring Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Massage_BYTEOFFSET,Massage_BITOFFSET,Massage_LEN,(uint8_t *)devStatusPtr,dataPoints->valueMassage);
    if(dataPoints->valueSleep >= Sleep_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valueSleep Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(Sleep_BYTEOFFSET,Sleep_BITOFFSET,Sleep_LEN,(uint8_t *)devStatusPtr,dataPoints->valueSleep);  
    }
    ...
    devStatusPtr->valueHeart_rate = gizY2X(Heart_rate_RATIO,  Heart_rate_ADDITION, dataPoints->valueHeart_rate); 
    devStatusPtr->valueBlood_oxygen = gizY2X(Blood_oxygen_RATIO,  Blood_oxygen_ADDITION, dataPoints->valueBlood_oxygen); 

    devStatusPtr->valueSnoring = exchangeBytes(gizY2X(Snoring_RATIO,  Snoring_ADDITION, dataPoints->valueSnoring)); 
